...
- 처음에는 API 요청을 서버에서 캐싱하는 방법으로 접근하여 `request header`의 `cache-control` 을 설정하려고 했으나 CORS 이슈가 발생했고
  ...
  그래서 요청 결과를 로컬 스토리지에 저장한 후 동일한 요청에 대해서 로컬 스토리지의 데이터를 읽어오도록 로컬 캐싱을 구현했습니다.
...
- `onFocus`와 `onBlur` 대신 `window`에 `click` 이벤트를 등록한 후 `event.target`이 `search` 컴포넌트에 속하지 않으면 목록이 보이지 않게 구현했습니다.
